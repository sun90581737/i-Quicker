...
$(function ($) {
    ...
    $("#content-wrapper").find('.mainContent').height($(window).height()-60);
    ...
    $('body').find('#make-small-nav').click(function (e) {
        ...
        if ($('#temp_ico').css("display")=="none") {
            $('#temp_ico').css("display", "block");
        } else if ($('#temp_ico').css("display") == "block") {
            $('#temp_ico').css("display", "none");
        }
    });
...
function GetLoadNav() {
    ...
    var _html = "";
    _html += '<div id="temp_ico" style="text-align:center;margin-top:5px;display:none">'
    _html += '<img src="/Content/img/i-Quicker/bitbug_favicon.ico">'
    _html += '</div>'
        ...
        if (row.F_ParentId == "0") {
            ...
            if (childNodes.length > 0) {
                ...
                $.each(childNodes, function (i) {
                    ...
                    _html += '<li>';
                    //if (subrow.F_UrlAddress.lastIndexOf("ReportManage") != -1) {
                    //    _html += '<a target="_blank" data-id="' + subrow.F_Id + '" href="' + subrow.F_UrlAddress + '" data-index="' + subrow.F_SortCode + '">' + subrow.F_FullName + '</a>';
                    //}
                    //else {
